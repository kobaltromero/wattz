...
import io.github.kobaltromero.wattz.registry.*;
...
public class Wattz {
    ...
    public Wattz(IEventBus modEventBus, ModContainer modContainer) {
        ...
        WattzConditions.CONDITIONS.register(modEventBus);
        WattzCreativeModeTabs.TABS.register(modEventBus);
    ...
    private void commonSetup(FMLCommonSetupEvent event) {
        ...
        BlockStressValues.IMPACTS.register(WattzBlocks.CRUDE_ALTERNATOR.get(), () -> Config.getMaxStress() / 256.0);
        ...
        for (WattzBlocks.CasingType casing : WattzBlocks.CasingType.values()) {
            ...
            BlockStressValues.IMPACTS.register(crudeEncased, () -> Config.getMaxStress() / 256.0);

            TooltipModifier.REGISTRY.register(WattzItems.getCrudeAlternatorEncased(casing).get(), KineticStats.create(WattzItems.getCrudeAlternatorEncased(casing).get()));
        }
        ...
        for (Tier.Alternator tier : Tier.Alternator.ALL) {
            ...
            TooltipModifier.REGISTRY.register(WattzItems.getAlternator(tier.id()).get(), KineticStats.create(WattzItems.getAlternator(tier.id()).get()));
            ...
            for (WattzBlocks.CasingType casing : WattzBlocks.CasingType.values()) {
                ...
                TooltipModifier.REGISTRY.register(WattzItems.getAlternatorEncased(tier.id(), casing).get(), KineticStats.create(WattzItems.getAlternatorEncased(tier.id(), casing).get()));
            }
        }
    }
